<?php
    ...
    $check1 = "select groupid from event5 where member1='$invoice1' or member2='$invoice1'";
    $res1a = mysqli_query($con, $check1);
    $row1a = mysqli_fetch_assoc($res1a);
    $gp1 = $row1a['groupid'];

    $check2 = "select groupid from event5 where member1='$invoice2' or member2='$invoice2'";
    ...
    if($invoice1=="" || $invoice2=="")
        header("location:Quiz.php?msg=Enter%20both%20order%20ID");
    else if($invoice1 == $invoice2)
        header("location:Quiz.php?msg=Same%20invoice%20number");
    //else if($status1!="TXN_SUCCESS" && $status!="TXN_SUCCESS")
      //      header("location:Quiz.php?msg=Please%20check%20your%20pyment%20status");
    else if($gp1=="" && $gp2=="")
        {
            $sql = "INSERT INTO event5(member1, member2) VALUES('$invoice1', '$invoice2')";
            mysqli_query($con, $sql);
            header("location:Quiz.php?msg=Team%20formation%20Successful");
        }
    
     else
            header("location:Quiz.php?msg=Team%20already%20exists");
